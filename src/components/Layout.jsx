...
export function Layout({ children, currentPage, onNavigate, onLogout }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-sky-100 via-blue-50 to-sky-200">
      {/* Mobile header */}
      <div className="lg:hidden fixed top-0 left-0 right-0 bg-white/95 backdrop-blur-md shadow-sm z-50 p-4 flex items-center justify-between border-b border-blue-100">
        <DriveLogo />
        <button
      ...
      <aside
        ...
        `}
      >
        <div className="p-6 hidden lg:block border-b border-blue-50">
          <DriveLogo />
        </div>

        <nav className="mt-20 lg:mt-8 px-4 space-y-2 flex-1">
          {navItems.map((item) => {
